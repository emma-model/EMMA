...
# List of scenarios with metadata (might be generated automatically at a later pint in time)
def read_scenarios(yaml_name):
    ...
    for ID, attributes in scens.items():
        if scens[ID]['clp'] is None:
            ...
            scens[ID]['clp'] = {'--ID': ID, **yaml_dict['default_clp'], **scens[ID]['clp']}
    return scens
...
def call(yaml_name):
    ...
    if toggle['load data']:
        ...
        if process.returncode != 0:
            print('ERROR: problem with executing these commands:', ' '.join(process.args))
            return
    ...
    for scenario_id, scenario_attributes in scenarios.items():
        if scenario_attributes['active']:
            ...
            print('Scenario is not flagged as active, hence skipping: id', scenario_id, scenario_attributes['name'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-y', '--yaml_file', help='Name of the yaml file to be used', type=str)  # default='default_val'
